...
import {AlertOctagonIcon, CheckCircleIcon} from "@zhuowenli/vue-feather-icons";
...
<template>
    <Layout>
        ...
        <b-row>
                ...
                <b-card class="card-height-100" no-body>
                    <b-card-body>
                        ...
                        <div class="progress animated-progress custom-progress mb-1">
                            <div aria-valuemax="100" aria-valuemin="0" aria-valuenow="38" class="progress-bar bg-info"
                                 role="progressbar" style="width: 38%"></div>
                        </div>
                        <p class="text-muted mb-2">You used 215 of 2000 of your API</p>
                        <div class="text-end">
                            <b-link id="createApi-btn" class="btn btn-success button create-btn btn-sm" type="button"
                                    @click="(e) => handleApikeydetails(e)">Create API Key
                            </b-link>
                        </div>
                    </b-card-body>
                </b-card>
            ...
            <b-col lg="4">
                <b-card class="card-animate card-height-100" no-body>
                    <b-card-body>
                        <div class="d-flex justify-content-between">
                            <div>
                                <p class="fw-medium text-muted mb-0">Successful conversions</p>
                                <h2 class="mt-4 ff-secondary fw-semibold">
                                    <count-to :duration='5000' :endVal='50' :startVal='0'></count-to>
                                </h2>
                                <p class="mb-0 text-muted">
                                    <b-badge class="bg-light text-success mb-0"><i
                                        class="ri-arrow-up-line align-middle"></i> 8.24 %
                                    </b-badge>
                                    7 last week
                                </p>
                            </div>
                  ...
                  <span class="avatar-title bg-soft-success rounded-circle fs-2">
                    <CheckCircleIcon class="text-success" height="24" width="24"></CheckCircleIcon>
                  </span>
                                </div>
                            </div>
                        </div>
            ...
            <b-col lg="4">
                <b-card class="card-animate card-height-100" no-body>
                    <b-card-body>
                        <div class="d-flex justify-content-between">
                            <div>
                                ...
                                <p class="mb-0 text-muted">
                                    <b-badge class="bg-light text-danger mb-0"><i
                                        class="ri-arrow-down-line align-middle"></i> 25.87 %
                                    </b-badge>
                                    7 last week
                                </p>
                            </div>
                  ...
                  <span class="avatar-title bg-soft-danger rounded-circle fs-2">
                    <AlertOctagonIcon class="text-danger" height="24" width="24"></AlertOctagonIcon>
                  </span>
                                </div>
                            </div>
                        </div>
        ...
        <b-row>
            <b-col lg="12">
                <b-card id="apiKeyList" no-body>
                    <b-card-header class="d-flex align-items-center">
                        <h5 class="card-title flex-grow-1 mb-0">API Keys</h5>
                        <div class="d-flex gap-1 flex-wrap">
                            <b-button id="remove-actions" onClick="deleteMultiple()" variant="soft-danger"><i
                                class="ri-delete-bin-2-line"></i>
                            </b-button>
                            <b-link id="addApi-btn" class="btn btn-success button create-btn" type="button"
                    ...
                    <b-card-body>
                            ...
                            <div class="table-responsive table-card mb-3">
                                <table class="table align-middle table-nowrap mb-0">
                                    ...
                                    <tr v-for="(data, index) of resultQuery" :key="index">
                                        ...
                                        <td class="apikey">
                                            <input :value="data.apikey" class="form-control apikey-value" readonly=""
                                                   type="text">
                                        </td>
                                        <td class="status">
                                            <b-badge :class="{
                        ...
                        'badge-soft-danger': data.status == 'Disable',
                      }" variant="badge-soft-success">{{ data.status }}
                                            </b-badge>
                                        </td>
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                <b-button aria-expanded="false" class="dropdown"
                                                          data-bs-toggle="dropdown"
                                                          size="sm"
                                                          type="button" variant="soft-secondary">
                                                    <i class="ri-more-fill align-middle"></i>
                                                </b-button>
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    <li>
                                                        <b-link id="edit-item-btn" class="dropdown-item edit-item-btn"
                                                                @click="(e) => handleApikeydetails(e, data)">Rename
                                                        </b-link>
                                                    </li>
                                                    <li>
                                                        <b-link id="regenerate-api-btn"
                                                                class="dropdown-item regenerate-api-btn"
                                                                @click="(e) => handleApikeydetails(e, data)">Regenerate
                                                            Key
                                                        </b-link>
                                                    </li>
                                                    <li>
                                                        <b-link id="disable-btn" class="dropdown-item disable-btn"
                                                    ...
                                                    <li>
                                                        <b-link class="dropdown-item remove-item-btn"
                                                                href="#deleteApiKeyModal"
                                                                @click="deleteApikey(data)">Delete
                                                        </b-link>
                                                    </li>
                                                </ul>
                                            </div>
        ...
        <!-- Modal -->
        <b-modal id="showModal" v-model="createApiModal" centered class="v-modal-custom"
                 hide-footer
                 no-close-on-backdrop title-class="exampleModalLabel">
            <b-form id="addform" autocomplete="off">
                <div id="api-key-error-msg" class="alert alert-danger py-2 d-none">Please enter api key name</div>
                <input id="apikeyId" type="hidden">
                <div class="mb-3">
                    <label class="form-label" for="api-key-name">API Key Name <span class="text-danger">*</span></label>
                    <input id="api-key-name" class="form-control" placeholder="Enter api key name" type="text">
                </div>
                <div id="apikey-element" class="mb-3" style="display: none;">
                    <label class="form-label" for="api-key">API Key</label>
                    <input id="api-key" class="form-control" disabled type="text" value="b5815DE8A7224438932eb296Z5">
                </div>
            </b-form>
            <div class="hstack gap-2 justify-content-end mt-3">
                <b-button id="close-modal" type="button" variant="secondary" @click="createApiModal = false">Close
                </b-button>
                <button id="createApimodal-btn" class="btn btn-primary" type="button"
                        @click="(e) => handleApikeydetails(e)">Create API
                </button>
                <b-button id="add-btn" type="button" variant="primary" @click="handleApiKeys">Add</b-button>
                <b-button id="edit-btn" type="button" variant="primary" @click="handleApiKeys">Save Changes</b-button>
            </div>
        </b-modal>
    </Layout>
</template>
